...
bool   PREDICTOR::GetPrediction(UINT32 PC) {
    ...
    UINT32 bimodalEntries;
    bimodalEntries = ( 1 << BIMODAL_BITS) ;
 ...
 // Folding(&folded_index[0], globalHistory);
  ...
  for ( int j = 0 ; j < 2 ; j ++)
  {
     	for (int i = 0; i < NUMBER_OF_TAGE; ++i) {
          ...
          folded_tag[j][i].comp = (folded_tag[0][i].comp << 1) | globalHistory[0];
          folded_tag[j][i].comp ^= globalHistory[folded_tag[0][i].orig_length] << (folded_tag[0][i].orig_length % folded_tag[0][i].comp_length);
          folded_tag[j][i].comp ^= (folded_tag[0][i].comp >> folded_tag[0][i].comp_length);
          folded_tag[j][i].comp &= (1 << folded_tag[0][i].comp_length) - 1;
  ...
  {
    /* code */
    folded_index[i].comp = (folded_index[i].comp << 1) | globalHistory[0];
    folded_index[i].comp ^= globalHistory[folded_index[i].orig_length] << (folded_index[i].orig_length % folded_index[i].comp_length);
    folded_index[i].comp ^= (folded_index[i].comp >> folded_index[i].comp_length);
    folded_index[i].comp &= (1 << folded_index[i].comp_length) - 1;
